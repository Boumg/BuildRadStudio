...
Nom = 'RAD Studio'
VersionRad = { "12.0":"xe5","13.0":"xe6", "14.0":"xe7", "15.0":"xe8",  "16.0":"xe9", "17.0":"seattle", "18.0":"berlin", "19.0":"tokyo", "20.0":"xe13?"}
...
class Embarcadero(object):
    ...
    @classmethod
    def getVersions(c):
       return [a for a in sorted(VersionRad.items())  if Embarcadero._DelphiRuche.siExisteRuche(a[0] + "\\C++")]
          
    @classmethod
    def getDerniereVersion(c):
        c.Versions = Embarcadero.getVersions()
        if len(c.Versions) == 0 :
            raise Exception("Embarcadero","Pas de version trouvee de Rad studio")
        return c.Versions[-1]

    def __init__(self):
         self.Version = Embarcadero.getDerniereVersion()
         self._RucheVersion = Embarcadero._DelphiRuche.getRegistre(self.Version[0])
    ...
    def getrsvars(s):
          ...
          with open(fichier, 'r') as mon_fichier:
                v= [ ligne.replace("@SET",'').strip().split("=") for ligne in mon_fichier.read().splitlines() if ligne != ""]
                for e in v:
                    e[0]=e[0].upper()
                return v
